...
{
    public class Desert_Slime : ModNPC
    {
        public override void SetStaticDefaults()
        {
            DisplayName.SetDefault("Desert Elemental Slime");
        ...
        private const int jumpDelay = 150;
        ...
        public override void AI()
        {
            ...
            if (npc.velocity.Y == 0)
            {
                ...
                {
                    npc.ai[2] = 0;
                    Main.PlaySound(new LegacySoundStyle(2, 100), npc.Center);
                    for (int j = 0; j < 50; j++)
                    {
                        Dust.NewDust(npc.position, npc.width, npc.height, DustID.Sandnado);
                    }
                    int proj = Projectile.NewProjectile(npc.Center, new Microsoft.Xna.Framework.Vector2(-3, 0), ProjectileID.SandnadoHostile, 15, 2);
                    Main.projectile[proj].tileCollide = false;
                    Main.projectile[proj].timeLeft = 180;
                    int proj2 = Projectile.NewProjectile(npc.Center, new Microsoft.Xna.Framework.Vector2(3, 0), ProjectileID.SandnadoHostile, 15, 2);
                ...
                {
                    npc.ai[3]++;
                    npc.velocity = npc.ai[3] % 3 == 0 ? new Microsoft.Xna.Framework.Vector2(npc.direction * 5, -12) : new Microsoft.Xna.Framework.Vector2(npc.direction * 5, -8);
                    npc.netUpdate = true;
                }
                npc.ai[0]++;
            }
        ...
        {
            if (!NPC.AnyNPCs(ModContent.NPCType<Fire_Slime>()) && !NPC.AnyNPCs(ModContent.NPCType<Ice_Slime>()) && !NPC.AnyNPCs(ModContent.NPCType<Earth_Slime>()))
            {
                if (Main.expertMode)
                {
