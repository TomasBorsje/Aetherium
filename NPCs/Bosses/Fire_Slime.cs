...
{
    public class Fire_Slime : ModNPC
    {
        public override void SetStaticDefaults()
        {
            DisplayName.SetDefault("Fiery Elemental Slime");
        ...
        private const int jumpDelay = 225;
        public override void AI()
        {
            ...
            if (npc.velocity.Y == 0)
            {
                ...
                {
                    npc.ai[2] = 0;
                    Main.PlaySound(new LegacySoundStyle(2, 45), npc.Center);
                    for (int j = 0; j < 50; j++)
                    {
                        Dust.NewDust(npc.position, npc.width, npc.height, 127);
                    }
                    for (int i = 0; i < 5; i++)
                    {
                        if (i == 2) { continue; }
                        int proj = Projectile.NewProjectile(npc.position + new Microsoft.Xna.Framework.Vector2(npc.width * (i / 5f), 0), new Microsoft.Xna.Framework.Vector2(3.5f*i - 7, 5), ProjectileID.BallofFire, 15, 2);
        ...
        {
            if (!NPC.AnyNPCs(ModContent.NPCType<Ice_Slime>()) && !NPC.AnyNPCs(ModContent.NPCType<Desert_Slime>()) && !NPC.AnyNPCs(ModContent.NPCType<Earth_Slime>()))
            {
                if (Main.expertMode)
                {
